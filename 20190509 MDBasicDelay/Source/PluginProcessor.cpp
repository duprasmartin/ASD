...
#endif
{
    ...
    mDryWet = 0;         // MD
    //mFeedbackAmount = 0; // MD

    
    mFeedbackLeft = 0; // instantiate the delay values to 0 bbefore anything goes in
    mFeedbackRight = 0; // MD
    ...
    addParameter (mFeedbackParameter = new AudioParameterFloat("delayfeedback",
                                                               "Delay Feedback",
                                                               0.,
                                                               0.99,
                                                               0.1));
    ...
    addParameter (mDelayOutputLevel = new AudioParameterFloat("delaylevel",
                                                              "Delay Level",
                                                              0.,
                                                              1.,
                                                              1.));
...
//        mCircularBufferRight[mDelayWriteHead] = rightChannel[i];
        ...
        float delay_sample_left  = mCircularBufferLeft[(int)mDelayPlayHead]; // MD store the left delay values
        float delay_sample_right = mCircularBufferRight[(int)mDelayPlayHead]; // MD ditto
        
        mFeedbackLeft  = delay_sample_left  * *mFeedbackParameter; // MD 0.8 is my feedback
        mFeedbackRight = delay_sample_right * *mFeedbackParameter; // MD ditto
        ...
        // MD removed to add our dry wet business
//        buffer.addSample(0,i, mFeedbackLeft );
//        buffer.addSample(1,i, mFeedbackRight);
//        float wetfactor = 0.9 ; // MD and I ask: why the hell not?

        buffer.addSample(0,i,
                         leftChannel[i] * (1-mDryWet) +
                         mFeedbackLeft  *    mDryWet  );
        
        buffer.addSample(1,i,
                         rightChannel[i] * (1-mDryWet) +
                         mFeedbackRight  *    mDryWet);
